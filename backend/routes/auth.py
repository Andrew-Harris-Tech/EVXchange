...
def login():
    ...
    return (
        "<h1>Authentication required</h1><p>Please login with one of the supported OAuth providers.</p>",
        401,
    )
    ...
    try:
        ...
        if user:
            ...
            if existing_user:
                ...
                existing_user.profile_picture = user_info.get(
                    "picture", existing_user.profile_picture
                )
                if user_info.get("verified_email"):
        ...
        provider_info.append(
            {
                "name": provider,
                "login_url": url_for(
                    "auth.oauth_login", provider=provider, _external=True
                ),
            }
        )
