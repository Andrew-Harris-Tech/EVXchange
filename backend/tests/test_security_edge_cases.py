...
class TestOAuthSecurity:
    ...
    def test_malformed_token_response(self, client):
        ...
        with patch(
            "services.oauth.OAuthService.get_provider", return_value=DummyProvider()
        ):
            response = client.get(
    ...
    def test_duplicate_oauth_account_linking(self, app):
        ...
        with app.app_context():
            # Create first user
            user1 = User(
                email="user1@test.com", name="User 1", google_id="duplicate_id"
            )
            ...
            user2 = User(
                email="user2@test.com", name="User 2", google_id="duplicate_id"
            )
...
class TestOAuthEdgeCases:
    ...
    def test_user_with_very_long_name(self, client, app):
        ...
        response = client.get(
            "/auth/callback/google?code=test_code&state=long_name_test"
        )
    ...
    def test_oauth_provider_returning_no_email(self, client, app):
        ...
        response = client.get(
            "/auth/callback/google?code=test_code&state=no_email_test"
        )
