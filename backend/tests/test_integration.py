...
class TestOAuthIntegration:
    ...
    def test_complete_google_oauth_flow(self, client, app):
        ...
        callback_response = client.get(
            f"/auth/callback/google?code=test_code&state={oauth_state}"
        )
    ...
    def test_multiple_provider_linking(self, client, app):
        ...
        fb_callback = client.get(
            f"/auth/callback/facebook?code=fb_code&state={facebook_state}"
        )
    ...
    def test_oauth_error_handling(self, client, app):
        ...
        response = client.get(
            "/auth/callback/google?code=bad_code&state=error_test_state"
        )
    ...
    def test_user_info_failure_handling(self, client, app):
        ...
        response = client.get(
            "/auth/callback/google?code=test_code&state=userinfo_test_state"
        )
        assert response.status_code == 500
        data = response.get_json()
        assert "Authentication failed" in data["error"]
