...
class TestAuthRoutes:
    ...
    def test_oauth_callback_google_existing_user(self, client, app):
        ...
        with app.app_context():
            existing_user = User(
                email="existing@gmail.com", name="Existing User", google_id="google123"
            )
    ...
    def test_oauth_callback_oauth_error(self, client):
        ...
        response = client.get(
            "/auth/callback/google?error=access_denied&state=test_state"
        )
    ...
    def test_oauth_callback_link_existing_email(self, client, app):
        ...
        with app.app_context():
            existing_user = User(email="same@gmail.com", name="Original User")
            db.session.add(existing_user)
            db.session.commit()
